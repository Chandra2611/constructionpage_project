...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        if ($id === '0') {
            ...
            $sql1 = "INSERT INTO `about_table` (`content`, `img`) 
                     VALUES ('$content', '$new_photo_name')";
            ...
            if (empty($new_photo_name)) {
                $sql_get_old_image = "SELECT img FROM `about_table` WHERE `id` = '$id'";
            ...
            $sql = "UPDATE `about_table` SET `content`='$content', `img`='$new_photo_name'
                     WHERE `id` = '$id'";
